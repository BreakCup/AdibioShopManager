...
@Component({
  selector: 'cashback',
  // template: '1',
  templateUrl: './cashback-table.component.html',
  styleUrls: ['./cashback-table.component.css']
})

export class CashbackTableComponent implements OnInit {
  ...
  displayedColumns = ['order_id', 'cashback','zhifubao','status']; //,'bank_card'
  ...
  ngOnInit() {
    ...
    this.router.data.subscribe(req=>{
      ...
      this.dataSource.refresh(this.cashbackData.parm);
